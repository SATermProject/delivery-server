...
public class ReviewRecommendationStrategy implements RecommendStrategy{
    ...
    public List<Object[]> recommend() {
            ...
            // 정렬된 restaurantId를 기준으로 해당 레스토랑 이름을 가져오기
            CriteriaBuilder cb = entityManager.getCriteriaBuilder();
            CriteriaQuery<Object[]> finalQuery = cb.createQuery(Object[].class);
            Root<Restaurant> root = finalQuery.from(Restaurant.class);

            finalQuery.select(cb.array(root.get("restaurantName")));
            finalQuery.where(root.get("restaurantId").in(sortedRestaurantIds));

            List<Object[]> resultList = entityManager.createQuery(finalQuery).getResultList();
            return resultList;
        }
}
